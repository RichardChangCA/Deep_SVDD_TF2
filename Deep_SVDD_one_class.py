...
def compute_loss(x, c, nu, eps = 1e-4):
    z = model.encode(x)
...
def train_step(inputs_image, c, optimizer, nu):
    # print("training......")

    with tf.GradientTape() as tape:
        loss, euclidean_norm = compute_loss(inputs_image, c, nu)
...
def customized_R_evaluation(train_images, batch_images, batch_labels, nu, c):
    ...
    return metric_results, dist_auc_roc
...
def train(train_images, test_images, test_labels, epochs, BATCH_SIZE, nu):
    ...
    for epoch in range(epochs):
        ...
        for index in range(0, len(train_images)-BATCH_SIZE, BATCH_SIZE):
            ...
            loss, optimizer = train_step(image_batch, c, optimizer, nu)
