...
def compute_loss(x, c, R, nu, eps = 1e-4):
    z = model.encode(x)
    ...
    return dist_loss, euclidean_norm
...
def train_step(inputs_image, c, optimizer, R, nu):
    # print("training......")

    with tf.GradientTape() as tape:
        loss, euclidean_norm = compute_loss(inputs_image, c, R, nu)
...
def customized_R_evaluation(train_images, batch_images, batch_labels, nu, c):
    ...
    # dist_auc_roc = sklearn.metrics.roc_auc_score(labels, dist_list)
    # print("dist_auc_roc:", dist_auc_roc)

    return metric_results, auc_roc

def train(train_images, test_images, test_labels, epochs, BATCH_SIZE, nu):
    ...
    for epoch in range(epochs):
        ...
        for index in range(0, len(train_images)-BATCH_SIZE, BATCH_SIZE):
            ...
            loss, optimizer, R = train_step(image_batch, c, optimizer, R, nu)

            # print("training Loss: ", loss)
        
        if(epoch % 10 == 0 and epoch >= warm_up_epochs):
...
train(train_images, test_images, test_labels, epochs, BATCH_SIZE, nu)