...
def compute_loss(x, c, alpha, beta, eps = 1e-4):
    z = model.encode(x)
...
def train_step(inputs_image, c, optimizer, alpha, beta):
    # print("training......")

    with tf.GradientTape() as tape:
        loss, _ = compute_loss(inputs_image, c, alpha, beta)
...
def train(train_images, test_images, test_labels, epochs, BATCH_SIZE, nu, alpha, beta):
    ...
    for epoch in range(epochs):
        ...
        for index in range(0, len(train_images)-BATCH_SIZE, BATCH_SIZE):
            ...
            loss, optimizer = train_step(image_batch, c, optimizer, alpha, beta)

            # print("training Loss: ", loss)
        
        if(epoch % 10 == 0 and epoch > autoencoder_pretrain_epochs):
