...
def compute_loss(x, c, alpha, beta, R, nu, eps = 1e-4):
    z = model.encode(x)
    ...
    penalty = tf.math.maximum(score, tf.zeros_like(score))

    dist_loss = tf.math.reduce_mean(R ** 2 + (1/nu)*penalty)
    ...
    loss = alpha * reconstruction_loss + beta * dist_loss
    # print("alpha * reconstruction_loss:", alpha * reconstruction_loss)
    print("beta * dist_loss:", beta * dist_loss)
...
def train_step(inputs_image, c, optimizer, alpha, beta, R, nu):
    # print("training......")

    with tf.GradientTape() as tape:
        loss, euclidean_norm = compute_loss(inputs_image, c, alpha, beta, R, nu)

        # print(loss)

    R = np.quantile(euclidean_norm, 1 - nu)
    ...
    return np.array(loss), optimizer, R
...
def get_R_based_on_c(train_images, nu, c, eps = 1e-4):
    ...
    for index in range(len(train_images)):
        ...
        latent_features = model.encode(image)
...
def customized_R_evaluation(train_images, batch_images, batch_labels, nu, c):
    ...
    auc_roc = sklearn.metrics.roc_auc_score(labels, scores_list)
...
def train(train_images, test_images, test_labels, epochs, BATCH_SIZE, nu, alpha, beta):

    global learning_rate
    global optimizer

    best_auc_roc = 0
    c = tf.Variable(np.zeros([latent_dim]), dtype=tf.float32, trainable=False)
    ...
    for epoch in range(epochs):
        ...
        if(epoch == autoencoder_pretrain_epochs): # end AE pre-train
            alpha = 0
            beta = 1
        ...
        for index in range(0, len(train_images)-BATCH_SIZE, BATCH_SIZE):
            ...
            if(epoch < warm_up_epochs):
                R = tf.Variable(0., dtype=tf.float32, trainable=False)
            loss, optimizer, R = train_step(image_batch, c, optimizer, alpha, beta, R, nu)

            # print("training Loss: ", loss)
        
        if(epoch % 1 == 0 and epoch >= autoencoder_pretrain_epochs):
            ...
            if(auc_roc > best_auc_roc):
                ...
                f = open("best_auc_roc_deep_svdd.txt", "w")
    ...
    np.save("c_deep_svdd.npy", np.array(c)) # save c
...
alpha = 1
beta = 0
train(train_images, test_images, test_labels, epochs, BATCH_SIZE, nu, alpha, beta)